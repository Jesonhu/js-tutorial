...
上面代码正确运行，输出了3次`abc`。这表明函数内部的变量`i`与循环变量`i`不在同一个作用域，有各自单独的作用域。[BackTop](#let知识点)
...
```javascript
// var 的情况
console.log(foo); // 输出undefined 执行顺序：1 var foo 2 console.log(foo); 3 foo = 2; 
var foo = 2;
...
上面代码中，变量`foo`用`var`命令声明，会发生变量提升，即脚本开始运行时，变量`foo`已经存在了，但是没有值，所以会输出`undefined`。变量`bar`用`let`命令声明，不会发生变量提升。这表示在声明它之前，变量`bar`是不存在的，这时如果用到它，就会抛出一个错误。[BackTop](#let知识点)
...
上面代码中，在`let`命令声明变量`tmp`之前，都属于变量`tmp`的“死区”。没有使用let声明的变量不会出现这个问题
...
总之，暂时性死区的本质就是，只要一进入当前作用域，所要使用的变量就已经存在了，但是不可获取，只有等到声明变量的那一行代码出现，才可以获取和使用该变量。[BackTop](#let知识点)
...
1. [为什么需要块级作用域](#为什么需要块级作用域？)
   避免外层变量被污染
   声明提前  
   for计数变量向外泄露

2. [ES6的块级作用域](#es6-的块级作用域)
...
第一种场景，内层变量可能会覆盖外层变量。\(**污染外层变量**\)

```javascript
var tmp = new Date();

function f() {
  console.log(tmp); // 由于内部存在tmp就使用内部的tmp
  if (false) {
    var tmp = 'hello world';
  }
}
...
上面代码中，变量`i`只用来控制循环，但是循环结束后，它并没有消失，泄露成了全局变量。[BackTop](#let知识点)
...
**块级作用域的出现，实际上使得获得广泛应用的立即执行函数表达式（IIFE）不再必要了。 **[**BackTop**](#块级作用域知识点)
...
上面代码在 ES5 中\(ie10以下\)运行，会得到“I am inside!”，因为在`if`内声明的函数`f`会被提升到函数头部，实际运行的代码如下。
...
上面代码中，变量`x`会得到整个块级作用域的返回值。[BackTop](#let知识点)
...
`const`声明的常量，也与`let`一样不可重复声明。[backTop](#const知识)
...
ES5 只有两种声明变量的方法：`var`命令和`function`命令。ES6除了添加`let`和`const`命令，后面章节还会提到，另外两种声明变量的方法：`import`命令和`class`命令。所以，ES6 一共有6种声明变量的方法。[BackTop](#const知识)
